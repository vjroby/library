...
// 8. load the Router class and provide the url + extension
    $router = new Framework\Router(array(
        "url" => isset($_GET["url"]) ? $_GET["url"] : "books/getBooks",
...
catch (Exception $e)
{
    ...
    foreach ($exceptions as $template => $classes)
    {
        foreach ($classes as $class)
        {
            ...
            {
                if ($template == '404'){
                    Framework\Registry::get('httpRequest')->setResponseCode(\Framework\HttpRequest::HTTP_RESPONSE_NOT_FOUND)
                        ->createRequestHeader();
                }else{
                    Framework\Registry::get('httpRequest')->setResponseCode(\Framework\HttpRequest::HTTP_RESPONSE_INTERNAL_SERVER_ERROR)
                        ->createRequestHeader();
                }
